...
export abstract class Writer {
  ...
  protected updateProgress(amount: number, tokens?: Record<string, string | number>): void {
    if (this.progressBar) {
      this.progressBar.tick(amount, tokens);
    }
  }

  protected updateProgressAbsolute(ratio: number, tokens?: Record<string, string | number>): void {
    if (this.progressBar) {
  ...
  protected write(data: Buffer): void {
    try {
      ...
      console.error('Write error:', error.message, 'data type:', typeof data);
      throw e;
    }
  }
  ...
  async writeData(data: C64FileInfo[], options: WriterOptions = {}): Promise<void> {
    await this.open();
    await this.writeContent(data, options);
    await this.close();
  }

  abstract writeContent(data: C64FileInfo[], options: WriterOptions): Promise<void>;
