...
export class TapePulseGeneratorKernal extends TapePulseGenerator {
    ...
    async generatePulses(file: C64FileInfo, hdrCode?: Buffer): Promise<void> {
        ...
        this.generateHeaderBlock(file.name, file.type, file.startAddr, file.endAddr, false, hdrCode);
        this.generatePilot('Short');
        this.generateHeaderBlock(file.name, file.type, file.startAddr, file.endAddr, true, hdrCode);
    ...
    protected sendShortPulse() { this.sendCustomPulse(PULSE_SHORT); }
    protected sendMediumPulse() { this.sendCustomPulse(PULSE_MEDIUM); }
    protected sendLongPulse() { this.sendCustomPulse(PULSE_LONG); }
    ...
    protected generateHeaderBlock(filename: string, fileType: string, startAddr: number, endAddr: number, isRepeat: boolean, hdrCode: Buffer | undefined = undefined): void {
        if (hdrCode && hdrCode.length > 192 - 21) {
            throw new Error("Given header code is too large to fit in tape header");
        }
        const typeByte = CBM_FILE_TYPE_CODES[fileType] || 0x03;
        const filenameBytes = filename.padEnd(16, '\x20').slice(0, 16).split('').map(c => c.charCodeAt(0));
        const headerData = [typeByte, startAddr & 0xFF, startAddr >> 8, endAddr & 0xFF, endAddr >> 8, ...filenameBytes];
        ...
        for (let i = 0; i < 192; i++) {
            ...
            if (hdrCode) {
                ...
                if (i >= hdrCodeOffsetStart && (i - hdrCodeOffsetStart) < hdrCode.length) {
    ...
    protected generatePilot(length: 'Header' | 'Data' | 'Short' | number): void {
        var count = 0;
