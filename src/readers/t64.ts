...
export function CbmFileTypeToValue(fileType: string): number {
    for (let key in CBM_FILE_TYPES) {
        if (CBM_FILE_TYPES[key] == fileType) return Number.parseInt(key);
    }
    return 0;
}
export class T64Reader extends Reader {
    ...
    async read(): Promise<C64Info> {
        ...
        if (
            [0x0100, 0x0101, 0x0200].findIndex((t) => t == this.version) == -1
        ) {
            throw new Error(
                `Invalid T64 version: "0x${this.version.toString(16).padStart(4, '0')}"`
            );
        }
        ...
        const versionStr =
            this.version === 0x0100
                ...
                : this.version === 0x0101
                  ...
                  : this.version === 0x0200
                    ? 'v2.0'
                    : `v${this.version & 0xff}.${(this.version >> 8) & 0xff}`;
