...
function showFormats(): void {
  const FORMAT_INFO = [
    { type: 'T64 Reader', ext: '.t64', desc: 'Commodore 64 digitized tape image format' },
    { type: 'PRG Reader', ext: '.prg', desc: 'Commodore 64 raw program' },
    { type: 'P00 Reader', ext: '.p00', desc: 'Commodore 64 raw program' },

    { type: 'T64 Writer', ext: '.t64', desc: 'Commodore 64 digitized tape image format' },
    { type: 'WAV Writer', ext: '.wav', desc: '48kHz, 8-bit mono audio' },
    { type: 'TAP Writer', ext: '.wav', desc: 'Commodore 64 raw tape format' },
  ...
  console.log(chalk.bold('─'.repeat(colWidths[0] + colWidths[1] + colWidths[2] + 7)));
  console.log(
...
program
  .name('c64util')
  .description('Convert Commodore 64 game/disk/tape formats to WAV audio files')
...
if (!options.input || options.help) {
  console.log(
    ...
    '  ' + chalk.yellow('-o, --output [file]') + '  Output file\n' +
    '  ' + chalk.yellow('-r, --rate [hz]') + '      Sample rate for WAV (default: 48000)\n' +
    '  ' + chalk.yellow('-t, --turbo') + '          Add turbo loader\n' +
    '  ' + chalk.yellow('-f, --files [mode]') + '   Files: auto (first), all, or number (default: auto)\n' +
    '  ' + chalk.yellow('-F, --formats') + '        List supported formats\n' +
    '  ' + chalk.yellow('-h, --help') + '           Show this help text\n\n' +
    chalk.bold('Supported Formats:') + ' T64, PRG, P00 → T64, TAP, WAV, PRG\n\n' +
    chalk.bold('Examples:') + '\n' +
    chalk.yellow(' c64util -i game.t64') + '                      Display information about the input file\n' +
    chalk.yellow(' c64util -i game.t64 -o game.wav') + '          Convert all files in game.t64 to game.wav\n' +
    chalk.yellow(' c64util -i game.t64 -o game.wav -t') + '       Convert all files using turbo loader\n' +
    chalk.yellow(' c64util -i game.t64 -o game.wav -r 44100') + ' Convert all files with 44.1kHz sample rate\n' +
    chalk.yellow(' c64util -i game.t64 -o game.tap -f 1') + '     Convert first file to TAP format\n'
  );
  process.exit(0);
}
...
function formatInfo(info: C64Info): string {
  ...
  if (info.type == 'T64') {
    ...
    lines.push('');
    lines.push(chalk.bold('  Version:') + '     ' + chalk.white(info.version));
    lines.push(chalk.bold('  Entries:') + '     ' + chalk.white(`${info.usedEntries} of ${info.maxEntries} used`));
    lines.push(chalk.bold('  Description:') + ' ' + chalk.white(info.description || 'None'));
    lines.push('');
  ...
  lines.push('');
  lines.push(chalk.bold('─────────────────────────────────────────────'));
  lines.push(chalk.bold('  Total:') + ' ' + chalk.white(`${info.totalBytes} bytes across ${info.files.length} file(s)`));
...
try {
  writer = WriterFactory.getWriter(options.output, { wavSampleRate: parseInt(options.rate), wavTurbo: options.turbo });
}
catch (err) {
  const error = err as Error;
...
if (options.files === 'auto') {
  if (writer.supportsMultipleFiles())
    filesToWrite = info.files;
  else
    filesToWrite = [info.files[0]];
  ...
  if (isNaN(fileIndex) || fileIndex < 1 || fileIndex > info.files.length) {
    console.log(chalk.red('✗ Invalid file index: ') + options.files + ' (must be 1-' + info.files.length + ')');
    process.exit(1);
  }
...
try {
  ...
  console.log('\n' + chalk.green('✓ Successfully wrote to ') + chalk.bold(options.output));
  console.log('  ' + chalk.yellow('Size:') + ' ' + sizeBytes + ' bytes (' + sizeMB + ' MB)');
  writer.printInfo();
} catch (err) {
  const error = err as Error;
